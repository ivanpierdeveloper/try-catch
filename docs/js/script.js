...
if (document.querySelector('.cubo-di-rubik-js')) {
    ...
    btn_customized_js.addEventListener('click', function() {
        try {
            ...
            fnc.showMyAlert("avviso", `${Exception.message} ${Exception.name.toLowerCase()}`, 'var(--danger)', 'var(--white)', 'var(--dark)', 'var(--warning)')
        }
    });
}
// ./button customized JS
...
if (document.querySelector('.cubo-di-rubik-json')) {
    ...
    button_json.addEventListener('click', async function() {
        ...
        */
        ...
        let fd = new FormData();
        fd.append("id", 11);
        let url = "docs/pages/json.json"; //"docs/pages/json.json";
        ...
        await fetch(rq)
            ...
            .then((data) => {
                ...
                data.forEach(function(val) {
                    ...
                    console.warn(val.content.firstname);
                    console.error(val.content.content2.mail);
